...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const body = req.body || {};
    const method = body.method === 'card' ? 'UNDEFINED' : 'PIX';
    const now = new Date();
    ...
    const existing = await sb(`/rest/v1/payments?student_id=eq.${encodeURIComponent(user.id)}&due_date=eq.${encodeURIComponent(due)}&status=in.(pending,confirmed)&select=id,external_id,invoice_url&limit=1`);
    const existingData = await existing.json();
    if (existing.ok && existingData?.[0]?.invoice_url) return res.status(200).json({ ok: true, invoiceUrl: existingData[0].invoice_url, paymentId: existingData[0].external_id });
    ...
    await sb('/rest/v1/payments', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify({ student_id: user.id, amount: Number(student.monthly_value), due_date: due, method: method === 'PIX' ? 'Pix' : 'Cartão', status: 'pending', external_id: payment.id, invoice_url: payment.invoiceUrl || null }) });
    return res.status(200).json({ ok: true, paymentId: payment.id, invoiceUrl: payment.invoiceUrl || null, billingType: method });
  } catch (error) {
