...
module.exports = async (req, res) => {
  ...
  const [year,month,day]=brDate.split('-').map(Number), today=new Date(Date.UTC(year,month-1,day));
  ...
  try{
    const [sr,pr,payr]=await Promise.all([
      ...
      api(`/rest/v1/payments?due_date=gte.${period}-01&due_date=lt.${nextMonthIso}&select=student_id,due_date,status`)
    ]);
    const students=await sr.json(),profiles=await pr.json(),payments=await payr.json();
    ...
    for(const student of students||[]){
      const profile=profileMap.get(student.id);if(!profile){result.skipped++;continue;}
      const due=dueDateFor(student.due_day),dueIso=iso(due),diffDays=Math.round((due.getTime()-today.getTime())/86400000),paid=paidSet.has(`${student.id}:${dueIso}`);
      const nextStatus=paid?'paid':(diffDays<0?'late':'pending');
      if(student.status!==nextStatus){const u=await api(`/rest/v1/students?id=eq.${encodeURIComponent(student.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:nextStatus})});if(!u.ok)throw new Error('Não foi possível atualizar o status mensal.');result.statusUpdated++;}
      if(paid){result.skipped++;continue;}
      let type=null,title=null,message=null;
      if(diffDays===3){type='payment_reminder_3d';title='Mensalidade próxima do vencimento';message=`Sua mensalidade de R$ ${money(student.monthly_value)} vence em 3 dias (${dueIso}).`;}
      else if(diffDays===0){type='payment_due_today';title='Mensalidade vence hoje';message=`Sua mensalidade de R$ ${money(student.monthly_value)} vence hoje (${dueIso}).`;}
      else if(diffDays<0){type='payment_overdue';title='Mensalidade em atraso';message=`Sua mensalidade de R$ ${money(student.monthly_value)} está em atraso desde ${dueIso}.`;result.overdue++;}
      if(!type){result.skipped++;continue;}
      const dedupeKey=`billing:${student.id}:${period}:${type}`;
      const nr=await api('/rest/v1/notifications',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({user_id:student.id,title,message,type,dedupe_key:dedupeKey})});
      if(nr.status===409){result.skipped++;continue;}if(!nr.ok)throw new Error('Não foi possível criar uma notificação.');result.created++;
    }
    return res.status(200).json({ok:true,date:brDate,...result});
  }catch(error){console.error('Dunamis Fit notifications-daily:',error);return res.status(500).json({error:error?.message||'Erro ao processar notificações.'});}
};
