...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const cancelledEvent = ['PAYMENT_OVERDUE', 'PAYMENT_CANCELLED', 'PAYMENT_REFUNDED'].includes(event.event) || ['OVERDUE', 'CANCELLED', 'REFUNDED'].includes(payment.status);
    if (!paidEvent && !cancelledEvent) return res.status(200).json({ ok: true, ignored: true, event: event.event });

    // Resolve the local payment first by the immutable gateway ID; externalReference is the fallback.
    const byGateway = await api(`/rest/v1/payments?gateway_payment_id=eq.${encodeURIComponent(paymentId)}&select=id,student_id,due_date,status,external_reference&limit=1`);
    ...
    if (!localPayment && external) {
      const byExternal = await api(`/rest/v1/payments?external_reference=eq.${encodeURIComponent(external)}&select=id,student_id,due_date,status,external_reference&limit=1`);
    ...
    const isPaid = paidEvent;
    const status = isPaid ? 'paid' : (cancelledEvent ? 'pending' : 'pending');
    const method = payment.billingType === 'PIX' ? 'Pix' : payment.billingType === 'CREDIT_CARD' ? 'Cartão' : (localPayment?.method || 'Pix');
    const paidAt = payment.paymentDate || payment.confirmedDate || (isPaid ? new Date().toISOString() : null);
    const paymentData = {
    ...
    if (localPayment?.id) {
      const update = await api(`/rest/v1/payments?id=eq.${encodeURIComponent(localPayment.id)}`, {
        method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(paymentData)
      });
      ...
      if (!insert.ok) {
        // A concorrência pode ter criado a mesma linha; reconcile it by the unique business key.
        const retry = await api(`/rest/v1/payments?student_id=eq.${encodeURIComponent(studentId)}&due_date=eq.${encodeURIComponent(dueDate)}`, {
          method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(paymentData)
        });
    ...
    if (isPaid) {
      ...
      const nr = await api('/rest/v1/notifications', {
        ...
        body: JSON.stringify({
          ...
          type: 'payment',
          dedupe_key: `payment:${paymentId}:student`
        })
      });
        ...
        for (const admin of adminRows || []) {
          const ar = await api('/rest/v1/notifications', {
            ...
            body: JSON.stringify({
              ...
              dedupe_key: `payment:${paymentId}:admin:${admin.id}`
            })
          });
