...
async function sendTemplate(to, amount, dueDate) {
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      ...
      template: { name: template, language: { code: language }, components: [{ type: 'body', parameters: [{ type: 'text', text: amount }, { type: 'text', text: dueDate }] }] }
    })
  });
...
module.exports = async function handler(request, response) {
  try {
    ...
    const { year, month, period } = currentMonth(today);
    ...
    for (const student of students || []) {
      ...
      const effectiveDueDay = Math.min(Number(student.due_day || 1), daysInMonth(year, month));
      if (day <= effectiveDueDay) { skipped++; continue; }
      const phone = normalizePhone(profile?.phone);
      if (!phone || phone.length < 12) { skipped++; results.push({ id: student.id, status: 'sem_whatsapp' }); continue; }

      const dedupeKey = `whatsapp_overdue:${student.id}:${period}`;
      const already = await supabaseRequest(`/rest/v1/notifications?select=id&dedupe_key=eq.${encodeURIComponent(dedupeKey)}&limit=1`);
      if (already?.length) { skipped++; continue; }

      const dueDate = `${year}-${String(month).padStart(2,'0')}-${String(effectiveDueDay).padStart(2,'0')}`;
      try {
        const sentMessage = await sendTemplate(phone, `R$ ${Number(student.monthly_value || 0).toLocaleString('pt-BR',{minimumFractionDigits:2})}`, dueDate);
        const messageId = sentMessage?.messages?.[0]?.id || null;
        const record = await fetch(`${process.env.SUPABASE_URL}/rest/v1/notifications`, {
          method: 'POST',
          ...
          body: JSON.stringify({ user_id: student.id, title: 'Cobrança enviada pelo WhatsApp', message: `Aviso de mensalidade em atraso enviado para ${profile?.full_name || 'aluno'}.${messageId ? ` ID: ${messageId}` : ''}`, type: 'whatsapp_overdue', dedupe_key: dedupeKey })
        });
        if (!record.ok && record.status !== 409) throw new Error('WhatsApp enviado, mas não foi possível registrar o histórico.');
        sent++; results.push({ id: student.id, status: 'enviado', messageId });
      } catch (error) {
        failed++; results.push({ id: student.id, status: 'erro', error: String(error.message || error).slice(0, 300) });
      }
    }
