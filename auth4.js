...
(function(){
  ...
  async function refreshCloudData(){
    ...
    if(current.role==='admin'){
      const [pr,st,ev,pa]=await Promise.all([sb.from('profiles').select('*').eq('role','student').order('full_name'),sb.from('students').select('*'),sb.from('evaluations').select('*').order('evaluation_date',{ascending:true}),sb.from('payments').select('*').order('due_date',{ascending:false})]);
      if(pr.error)throw pr.error;if(st.error)throw st.error;if(ev.error)throw ev.error;if(pa.error)throw pa.error;
      data.students=(pr.data||[]).map(p=>mapStudent(p,(st.data||[]).find(x=>x.id===p.id),(ev.data||[]).filter(x=>x.student_id===p.id),(pa.data||[]).filter(x=>x.student_id===p.id)));
  ...
  window.addEventListener('load',async()=>{const recovery=await handleRecovery();if(recovery)return;const ok=await loadSession();if(ok)render()});
})();
