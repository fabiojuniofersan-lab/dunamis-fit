...
(function(){
  ...
  window.pay=async function(id){
    ...
    try{
      ...
      const notification=await sb.from('notifications').insert({user_id:current.studentId,title:'Pagamento confirmado',message:`Pagamento confirmado: ${s.name} — R$ ${Number(s.value||0).toFixed(2).replace('.',',')}`,type:'payment'});
