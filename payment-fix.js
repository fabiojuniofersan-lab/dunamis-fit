...
(function(){
  ...
  window.pay=async function(id){
    ...
    try{
      ...
      await sb.from('notifications').insert({user_id:window.current.studentId,title:'Pagamento confirmado',message:`Pagamento confirmado: ${s.name} — R$ ${Number(s.value||0).toFixed(2).replace('.',',')}`,type:'payment'});
